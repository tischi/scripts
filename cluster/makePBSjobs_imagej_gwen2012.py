...
#expname = "Experiment-41-"
#python2.7 /g/almf/software/scripts/cluster/makePBSjobs_imagej_gwen2012.py --prg-binary /g/almf/software/bin2/fiji --prg-plugins-dir=/g/almf/software/fiji_tischi/plugins --prg-script=/g/almf/software/scripts/imagej/tischi_gwen--stitch_edf_headless--2012_10_19.py --data-in-dir=/g/almfscreen/Gwen/2012-08-16--tifs
...
def prepare_jobs(prg_binary, prg_plugins_dir, prg_script, data_in_dir, delete=True,
    ...
    if data_in_dir is None:
        ...
        output_dir = data_in_dir + '--clust_manage'

    print('Cluster management directory: {}'.format(output_dir))
    dirTxtOut = os.path.join(output_dir, "log")
    dirJobs = os.path.join(output_dir, "jobs")
    dirStatus = os.path.join(output_dir, "status")
    dirAnalysed = data_in_dir + '--clust_output'
    
    if not os.path.isdir(dirAnalysed):
    	print "Creating new folder "+dirAnalysed
	os.mkdir(dirAnalysed)
    else:
    	print "Cluster output directory: "+dirAnalysed+".."
    	print "..exists already; leaving it untouched; new data will be added!"
    ...
    for p in range(ps,pe+1):
        for t in range(ts,te+1,dt):
            ...
            # this only works if each job is only working on 1 position!!
            #expname = "Experiment-41-" # defined globally above
            cmd = [
                prg_binary,
                "--headless",
