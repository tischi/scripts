...
multipagepdf = function(d, cLabel, cX, cY, cTitle=NULL, path) {
  ...
  for (iObject in unique(d[[cLabel]])) {
    ...
    print(sTitle)
    title(paste(cLabel, iObject,"--", cTitle, sTitle))
  }
...
df = filterTracks(d, cLabel, measurement = "POSITION_T", minimum = 0, maximum = 0, function(z) max(z,na.rm=T))
...
multipagepdf(df, cLabel, cX = "Metadata_timepoint", cY = "Intensity_IntegratedIntensity_sumTophat", cTitle="Metadata_FileID", pathTable)
