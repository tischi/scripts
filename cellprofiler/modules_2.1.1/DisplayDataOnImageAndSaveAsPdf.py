'''<b>Display Data On Image And Save As pdf</b> 
produces an image with measured data on top of identified objects and directly saves it as an pdf.
<hr>
...
#
# Plugin developed by Christian Tischi Tischer (tischitischer@gmail.com)
# Tested for CellProfiler 2.1.1
# Based on the original DisplayDataOnImage module from CellProfiler
# Modifications:
# - added code to save the image as pdf
# Motivation:
# - if you paint numbers in an image they might not be visible if there are not enough pixels; 
#   pdf solves this problem as it can handle vector graphics (suggestion from Lee Kamensky)
#
...
class DisplayDataOnImageAndSaveAsPdf(cpm.CPModule):
    module_name = 'DisplayDataOnImageAndSaveAsPdf'
    category = 'Data Tools'
    ...
    def create_settings(self):
        ...
        self.single_file_name = cps.Text(
            "Filename (full path) for saving as pdf", "myFilename",
            metadata = True,  doc="""
            Specify the filename including the full path here (.pdf will be appended automatically).
            Saving the image as pdf has the advantage that the numbers will be nicely readable even 
            if the image does not have many pixels.
            """)
    ...
    def settings(self):
        ...
        return [self.objects_or_image, self.objects_name, self.measurement,
                ...
                self.font_size, self.decimals, self.saved_image_contents, self.single_file_name,
...
if __name__ == "__main__":
    ...
    tool_name = 'DisplayDataOnImageAndSaveAsPdf'
