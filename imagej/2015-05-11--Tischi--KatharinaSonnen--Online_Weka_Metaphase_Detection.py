...
class locDialogParameters(DialogParameters, ActionListener):
	...
	COMBO_ORDER = ['pip', 'task',	'channel', 'command', 	'clspath', 'nrPart', 'minProb', 'maxCirc', 'minArea', 'probThr']
	COMBO_SEPARATOR = [3]
...
def segmentChannel_Weka(image, **kwargs):
	...
	for i in range(len(Mean)):
		valid = (Mean[i]>kwargs['minProb']) and (Circ[i]<kwargs['maxCirc'] and (Area[i]>kwargs['minArea']) ) # filter particles post detection
		if(valid):
...
if test ==1 :
	...
	'Min Intensity':0.8, 'maxCirc': 0.8, 'minArea': 20, 'probThr':0.6}
